...
from datetime import datetime   #현재 시간을 파일 이름에 사용
import os       #폴더 만들기
import gstreamer
...
def main():
    ...
    FALL_THRESHOLD = 100  # Y 좌표 변화량 기준
    fall_detected = False
    ...
    def render_overlay(engine, output, src_size, inference_box, frame):
        print("[DEBUG] render_overlay called -> possiblity for stoping reder_loop in gstreamer")
        nonlocal n, sum_process_time, sum_inference_time, fps_counter
        nonlocal shoulder_y_history, fall_detected
        ...
        for pose in outputs:
            ...
            if ls and rs and ls.score > 0.5 and rs.score > 0.5:
                shoulder_y = (ls.point[1] + rs.point[1]) / 2
        ...
        if fall_detected:
            ...
            """
            # 이미지 저장 경로 설정
            SAVE_DIR = "fall_images"
            if not os.path.exists(SAVE_DIR):
                os.makedirs(SAVE_DIR)

            #현재 시간으로 파일 이름 설정
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = os.path.join(SAVE_DIR, f"fall_{timestamp}.jpg")

            #open cv로 이미지 저장
            cv2.imwrite(filename, frame)

            print("f fall image download done : {filename}")
            """
